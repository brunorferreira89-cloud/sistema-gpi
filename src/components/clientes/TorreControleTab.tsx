...
export function TorreControleTab({ clienteId }: Props) {
  ...
  const handleMetaSaved = useCallback(async (contaId: string, metaTipo: 'pct' | 'valor', metaValor: number | null) => {
    ...
    // Build an updated metaMap that includes the just-saved value
    ...
    if (conta.nivel === 2) {
      ...
      if (metaTipo === 'pct') {
        ...
        // Find the summed realized for this node
        const findNode = (nodes: DreNode[], id: string): DreNode | null => {
          for (const n of nodes) {
            if (n.conta.id === id) return n;
      ...
      for (const child of children) {
        ...
        // If grupo (nivel=0), also push grandchildren
        if (conta.nivel === 0) {
