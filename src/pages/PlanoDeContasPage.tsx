...
export function PlanoDeContasDetail({ clienteId, contas, onRefresh }: {
  ...
  const grupos = contas.filter((c) => c.nivel === 0);
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    await supabase.from('plano_de_contas').delete().eq('id', deleteId);
    // Also delete children
    const children = contas.filter((c) => c.conta_pai_id === deleteId);
    for (const child of children) {
      await supabase.from('plano_de_contas').delete().eq('conta_pai_id', child.id);
      await supabase.from('plano_de_contas').delete().eq('id', child.id);
    }
    setDeleteId(null);
    onRefresh();
  };
  ...
  return (
    <div className="space-y-0">
      {grupos.map((grupo) => {
        const subgrupos = contas.filter((c) => c.conta_pai_id === grupo.id && c.nivel === 1);

        return (
          <div key={grupo.id}>
            {/* GRUPO BAND — nivel 0 */}
            <div className="flex items-center justify-between bg-[#E5E7EB] px-4 py-2.5 border-b border-[#F3F4F6]">
              <div className="flex items-center gap-2 min-w-0">
                {editingId === grupo.id ? (
                  ...
                  <span className="text-xs font-semibold uppercase tracking-wide text-txt">{grupo.nome}</span>
                )}
              </div>
              <div className="flex items-center gap-1">
                <button onClick={() => startEdit(grupo.id, grupo.nome)} className="p-1 text-txt-muted hover:text-txt rounded hover:bg-black/5" title="Editar nome">
                  <Pencil className="h-3.5 w-3.5" />
                </button>
                <button onClick={() => startAdd(grupo.id, 1)} className="p-1 text-txt-muted hover:text-txt rounded hover:bg-black/5" title="Adicionar subgrupo">
            ...
            {/* Add subgrupo inline */}
            {addingParentId === grupo.id && addingNivel === 1 && (
              <div className="flex items-center gap-2 px-4 py-2 bg-primary/5 border-b border-[#F3F4F6]">
                <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Nome do subgrupo" className="h-7 text-xs w-60" autoFocus onKeyDown={(e) => e.key === 'Enter' && saveAdd()} />
            ...
            {/* SUBGRUPOS — nivel 1 */}
            {subgrupos.map((sub) => {
              const categorias = contas.filter((c) => c.conta_pai_id === sub.id && c.nivel === 2);

              return (
                <div key={sub.id}>
                  <div className="flex items-center justify-between px-4 py-2.5 bg-white border-b border-[#F3F4F6]">
                    <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => startEdit(sub.id, sub.nome)} className="p-1 text-txt-muted hover:text-txt rounded hover:bg-surface-hi" title="Editar nome">
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => startAdd(sub.id, 2)} className="p-1 text-txt-muted hover:text-txt rounded hover:bg-surface-hi" title="Adicionar categoria">
                        <Plus className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => setDeleteId(sub.id)} className="p-1 text-txt-muted hover:text-destructive rounded hover:bg-destructive/10" title="Excluir subgrupo">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </div>
                  </div>

                  {/* Add categoria inline */}
                  {addingParentId === sub.id && addingNivel === 2 && (
                    <div className="flex items-center gap-2 pl-[56px] pr-4 py-2 bg-primary/5 border-b border-[#F3F4F6]">
                      <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Nome da categoria" className="h-7 text-xs w-60" autoFocus onKeyDown={(e) => e.key === 'Enter' && saveAdd()} />
                      <button onClick={saveAdd} className="p-1 text-green hover:bg-green/10 rounded"><Check className="h-3.5 w-3.5" /></button>
                      <button onClick={() => setAddingParentId(null)} className="p-1 text-red hover:bg-red/10 rounded"><X className="h-3.5 w-3.5" /></button>
                    </div>
                  )}
                  ...
                  {/* CATEGORIAS — nivel 2 */}
                  {categorias.map((cat) => {
                    // Detect prefix from nome or default based on tipo
                    const isEntrada = cat.tipo === 'receita';

                    return (
                      <div key={cat.id} className="flex items-center justify-between pl-10 pr-4 py-2 bg-white border-b border-[#F3F4F6]">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {editingId === cat.id ? (
                            ...
                            <span className="text-sm text-txt">{cat.nome}</span>
                          )}
                        </div>
                        <div className="flex items-center gap-1">
                          <button onClick={() => startEdit(cat.id, cat.nome)} className="p-1 text-txt-muted hover:text-txt rounded hover:bg-surface-hi" title="Editar nome">
                            <Pencil className="h-3.5 w-3.5" />
                          </button>
                          <button onClick={() => setDeleteId(cat.id)} className="p-1 text-txt-muted hover:text-destructive rounded hover:bg-destructive/10" title="Excluir categoria">
                            <Trash2 className="h-3.5 w-3.5" />
                          </button>
                        </div>
                      </div>
